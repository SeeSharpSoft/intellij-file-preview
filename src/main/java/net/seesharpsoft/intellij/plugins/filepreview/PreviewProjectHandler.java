...
import static net.seesharpsoft.intellij.plugins.filepreview.PreviewFileEditorManager.FILE_OPENING_BEHAVIOR_KEY;
...
public class PreviewProjectHandler {
    ...
    public static boolean isPreviewFile(VirtualFile file) {
        return file != null && file.getUserData(PREVIEW_VIRTUAL_FILE_KEY) != null;
    }
        ...
        public void selectionChanged(@NotNull FileEditorManagerEvent event) {
            if (!PreviewSettings.getInstance().getPreviewBehavior().equals(EXPLICIT_PREVIEW)) {
                consumeSelectedFile(myProjectViewPane.getTree(), file -> {
                    if (PreviewSettings.getInstance().isPreviewClosedOnTabChange() || !isPreviewFile(file)) {
                        closePreviews();
                    }
                });
            }
    ...
    private final FileEditorManagerListener.Before myFileEditorManagerBeforeListener = new FileEditorManagerListener.Before() {
        ...
        public void beforeFileOpened(@NotNull FileEditorManager source, @NotNull VirtualFile file) {
            consumeSelectedFile(myProjectViewPane.getTree(), selectedFile -> {
                if (isPreviewFile(file) || (selectedFile != null && selectedFile.equals(file) && !PreviewSettings.getInstance().getPreviewBehavior().equals(EXPLICIT_PREVIEW))) {
                    closePreviews();
                }
    ...
    public void openPreviewOrEditor(VirtualFile file) {
        ...
        if (!fileEditorManager.isFileOpen(file)) {
            file.putUserData(FILE_OPENING_BEHAVIOR_KEY, PreviewFileEditorManager.FileOpeningBehavior.PREVIEW);
        }
